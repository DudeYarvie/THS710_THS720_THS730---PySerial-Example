##						THS720P_Scope_Acquisition.py
##Purpose: Provide a reference to conrol THS7XXX series oscilloscopes using Python.
##		   This code retrieves waveforms from specified channels on a Tektronix THS7XXX
...
##GLOBALS
act_probe_chs = ['1']                               ##List of active scope channels
data_file_str = "data_file.txt"
...
def Config_serial_comms():
        ser = serial.Serial(port,baud,timeout =5)  ##Establishes serial connection with instrument
...
def ACQ_WFM(instr):
	...
	#Set data buffer size to retrieve when a wfm request is made (e.g. "CURVe?" cmd)  
        ...
        instr.write("DATA:STOP 20000"+EOL)			    ##Set stop index of measurement buffer data		
        time.sleep(0.1)

        
        
	#Request X&Y constants (these are needed to convert the digital data in the instrument buffer into analog values)
        instr.write("WFMPre:CH"+ch+":YZEro?"+EOL)
        YZE = float(read_data(instr))

        instr.write("WFMPre:CH"+ch+":YMUlt?"+EOL)
        YMU = float(read_data(instr))

        instr.write("WFMPre:CH"+ch+":YOFf?"+EOL)
